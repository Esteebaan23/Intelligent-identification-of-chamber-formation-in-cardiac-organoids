...
# ============================================================
#
# Folder layout matches Classification.py's load_image_paths():
#
#   BASE_DIR/
#     Chamber Forming/
...
#       CH1/   *.tif
#
# Note: files inside the CH3 folder are suffixed "CH3", and the matching
# file inside the CH1 folder has that same name with "CH3" swapped for
# "CH1" (e.g. Image_XY35_CH3(4).tif -> Image_XY35_CH1(4).tif).
...
def load_image_pairs(base_dir, class_label, channel_tag='CH3', target_tag='CH1'):
    ...
    listing as Classification.py's load_image_paths().

    Files inside the `channel_tag` folder (e.g. CH3) are suffixed with
    `channel_tag` (e.g. "...CH3(4).tif"). The matching file inside the
    `target_tag` folder (e.g. CH1) has that same suffix swapped for
    `target_tag` (e.g. "...CH1(4).tif"). Pairing is done by:
      1. Trying an exact basename match in target_dir first.
      2. Falling back to swapping channel_tag -> target_tag in the
         input filename and looking for that name in target_dir
    ...
    # Case-insensitive match on extension (.tif / .TIF / .Tif, etc.), same
    # spirit as Classification.py's glob but tolerant of case on Windows.
    input_files = sorted(
    ...
    for input_path in input_files:
        ...
        if not os.path.exists(target_path):
            # 2) fall back to swapping the file's channel tag
            #    (e.g. CH3 -> CH1)
...
def run_training_phase(name, gray_files, green_files, labels, epochs,
    ...
    warm_up(gen, gen_opt, [(1, IMG_SIZE, IMG_SIZE, 3)])
    warm_up(disc, disc_opt, [(1, IMG_SIZE, IMG_SIZE, 3), (1, IMG_SIZE, IMG_SIZE, 3)])
...
# Only one class is trained per run. GPU memory from the first phase is not
# reliably released within the same Python process, so training both
# "forming" and "nonforming" back-to-back can OOM on the second phase for
# large datasets. Set this to "forming" or "nonforming", then run the
# script twice (once per value) to train both models.
